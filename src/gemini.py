...
CHATLOGS = LANG_DIR / "chatlogs"
BOTLOGS = LANG_DIR / "botlogs"
CODELOGS = CODE_DIR / "codelogs"

class Chat:

    def __init__(self, API, model="gemini-3.1-flash-lite", sys="you are a helpful assistant"):
        self.api = API
        self.sys = sys
    ...
    def generate_prose(self, prompt, num_words=1000):
        
        formatted_prompt = f"Generate approximately {num_words} tokens of text based on the following summary: {prompt}"
...
class Automate:
    ...
    def __init__(self, API, sys_a="you are a succinct and helpful assistant", sys_b="you are an inquisitive mind", model_a="gemini-3.1-flash-lite", model_b="gemini-3.1-flash-lite"):
        ...
        self.topics = [line for line in open(LANG_DIR / "promptfiles/bottopics.txt", 'r')]
...
class Code:
    ...
    def __init__(self, API, model="gemini-3.1-flash-lite"):
        ...
        with open(CODE_DIR / "engine_rules.txt", 'r') as f: 
            sys_instruct = f.read()
    ...
    def save_history(self, history, filename): 
        ...
        path = CODELOGS / filename
        master = CODELOGS / "codemaster.csv"
        with open(path, 'w', encoding='utf-8', newline='') as f, open(master, 'a', encoding='utf-8', newline='') as m: 
            writer = csv.writer(f)
            master_writer = csv.writer(m)
    ...
    def append_master(self, dataframe): 
        master_path = CODELOGS / "master_dataset.csv"
    ...
    async def generate_async_code(self, prompt, language="python"):

        prompt_instructions = [
                "Rewrite this code in your own style: ",
                "Write your own version of this program:",
                "Improve on and refactor this code, rewriting the entire thing in your own style: ", 
                "Create code similar to this without copying anything verbatim: ",
                "Make an alternate version of this code/logic: ",
                "Write an alternative approach to doing what this code is doing: "
                "Improve this program by doing a total rewrite: "
                ]
        
        instruction = random.choice(prompt_instructions)

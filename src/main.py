...
def menu():
    ...
    print("6. Create topic matched LLM and Human PROSE data")
    print("7. Create topic matched LLM and Human CODE data")
...
def cli():
    ...
    while (True): 
        ...
        if x.lower() == 'x':
            ...
            num_docs = int(input("\nEnter number of documents to create (per dataset): "))
            name = input("\nEnter name for project/paired datasets: ")

            ai_datafile, human_datafile = annotator.generate_topic_match_prose(num_docs=num_docs)

            make_dataset_dir(name, str(ai_datafile))
            make_dataset_dir(name, str(human_datafile))

            print(f"Human and ai datasets created and saved to {name}/ folder")

        elif x == '7':
            n_scripts = int(input("\nEnter number of scripts to create (per class/dataset): "))
            dirname = input("\nEnter name for project/paired datasets directory: ")
            
            if n_scripts < 500:
                human_datafile, hum_rows, ai_datafile, ai_rows = annotator.generate_topic_match_code(dirname, n_scripts)

                print(f"{hum_rows} human code scripts and {ai_rows} ai code scripts created.")

                make_dataset_dir(dirname, str(human_datafile))
                make_dataset_dir(dirname, str(ai_datafile))

                print(f"Data files for Human and LLM saved to {dirname}/")
            else: 
                human_datafile, hum_rows, ai_datafile, ai_rows = annotator.generate_async_topic_match_code(dirname, n_scripts)
